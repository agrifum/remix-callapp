...
| **T-TEL-04** | DOCUMENTED FACT | `Call.Details` in `CallScreeningService` exposes only 5 properties: `getCallDirection()`, `getCallerNumberVerificationStatus()`, `getConnectTimeMillis()`, `getCreationTimeMillis()`, and `getHandle()`. All other properties are null or default. | [CallScreeningService.onScreenCall](https://developer.android.com/reference/android/telecom/CallScreeningService#onScreenCall(android.telecom.Call.Details)) | API 24+ | HIGH | `Call.Details` is a static snapshot at placement time. `getConnectTimeMillis()` is 0 at `onScreenCall` execution because the call is not yet connected. `Call.Details` has no lifecycle listeners. | `CallScreeningService` cannot determine when an outgoing call is answered or connected. |
| **T-TEL-05** | DOCUMENTED FACT | `TelephonyManager.CALL_STATE_OFFHOOK` is defined as: "Device call state: Off-hook. At least one call exists that is dialing, active, or on hold, and no calls are ringing or waiting." | [TelephonyManager.CALL_STATE_OFFHOOK](https://developer.android.com/reference/android/telephony/TelephonyManager#CALL_STATE_OFFHOOK) | API 1+ (`TelephonyCallback` API 31–36) | HIGH | For outgoing calls, `CALL_STATE_OFFHOOK` fires immediately when dialing starts. There is NO state transition or callback when the remote party answers. The state remains `CALL_STATE_OFFHOOK` continuously until hangup. | `TelephonyManager` cannot distinguish between outgoing dialing and outgoing active conversation. |
...
| **T-TEL-08** | DOCUMENTED FACT | `InCallService` provides full real-time call states (`STATE_DIALING`, `STATE_CONNECTING`, `STATE_ACTIVE`, `STATE_DISCONNECTED`) and callbacks via `Call.Callback.onStateChanged`. However, binding requires `ROLE_DIALER`, `ROLE_CALL_COMPANION` (wearable companion), or privileged permission `MANAGE_ONGOING_CALLS`. | [InCallService](https://developer.android.com/reference/android/telecom/InCallService) & [MANAGE_ONGOING_CALLS](https://developer.android.com/reference/android/Manifest.permission#MANAGE_ONGOING_CALLS) | API 23+ / API 30+ / API 31–36 | HIGH | Prohibited by SP-001 and SP-049 ("CallUpp must not become the default dialer"); CallUpp is a phone app, not a companion wearable app. | The only documented Android API providing a real-time `STATE_ACTIVE` transition signal is unavailable within CallUpp's product boundary. |
| **T-TEL-09** | DOCUMENTED FACT | `CallLog.Calls` entries are inserted by Telecom only upon call termination (disconnect), because fields like `DURATION` (seconds) and final call disposition are unknown until the call finishes. | [CallLog.Calls.DURATION](https://developer.android.com/reference/android/provider/CallLog.Calls#DURATION) | API 1+ | HIGH | Zero rows exist for an ongoing active call. Cannot be queried in real time to establish whether an ongoing call is answered. | `CallLog` can only be queried at `CALL_STATE_IDLE` for retrospective reconciliation (e.g. checking `DURATION > 0` vs `DURATION == 0` to discard drafts from aborted outgoing dials). |
| **T-TEL-10** | DOCUMENTED FACT / PLATFORM LIMITATION | On Android API 31–36 (targetSdk 36), there is NO documented public Android API available to a non-default dialer application to detect the exact moment an outgoing call is answered by the remote party. | AOSP Telecom / Telephony Architecture & Android API Reference | API 31–36 | HIGH | The platform intentionally isolates remote-party answer events to the in-call subsystem (`InCallService`). Non-dialer apps only receive coarse `CALL_STATE_OFFHOOK`. | Detecting "actual active-call state" distinct from "dialing" for outgoing calls without replacing the default dialer is impossible via documented Android APIs. |
...
| **T-TEL-12** | INFERENCE | SP-008's requirement ("Overlay pojawia się dopiero, gdy rozmowa faktycznie trwa (OFFHOOK), nie musi być widoczny podczas samego dzwonienia") contains an internal platform impedance mismatch for outgoing calls. Showing the overlay on `CALL_STATE_OFFHOOK` satisfies the literal `OFFHOOK` condition, but contradicts "not merely while dialing". | Deduction from T-TEL-05, T-TEL-08, T-TEL-10, and `MASTER_SPEC.md` §8 | API 31–36 | HIGH | Architectural decision required between showing overlay during dialing vs not showing overlay until post-call or user manual trigger. | Control plane decision required on whether to accept dialing-time overlay appearance or adopt a fallback strategy. |
...
1. **Outgoing Overlay Trigger Policy:**
   Given that Android does not provide a signal for remote answer to non-dialer apps:
   - **Option 1 (Current behavior — Recommended for v1):** Show the overlay immediately upon `CALL_STATE_OFFHOOK` for outgoing calls (overlay appears during dialing). If the call ends with `DURATION == 0` in `CallLog` and no note text was typed, automatically discard the draft and dismiss the overlay.
   - **Option 2 (Post-Call Prompt):** Do not show an overlay during outgoing calls at all; instead, observe `CallLog` post-disconnect and display a heads-up notification/bottom sheet if `DURATION > 0` allowing immediate note creation.
   - **Option 3 (Collapsed Bubble during dialing):** Launch overlay in a minimal collapsed icon during `CALL_STATE_OFFHOOK` that expands upon user tap or auto-expands after a timer heuristic (e.g. 15 seconds).
2. **`READ_CONTACTS` Onboarding Requirement:**
   Because Telecom completely omits `CallScreeningService` screening for contact numbers without `READ_CONTACTS`, should onboarding treat `READ_CONTACTS` as a mandatory step in the primary telephony setup flow rather than an optional permission?
3. **Retirement of `PhoneStateReceiver`:**
   Should a cleanup prompt formally deprecate and remove `PhoneStateReceiver.kt` from the codebase, consolidating all telephony state tracking into `CallStateMonitor` (`TelephonyCallback`) and `CallScreeningServiceImpl`?
